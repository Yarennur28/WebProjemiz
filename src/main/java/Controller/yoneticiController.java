...
public class yoneticiController extends databaseConnection{
	...
	public List<yoneticiModel> readingData(){
	 	   ...
	 	   try {
	            ...
	            while(rs.next())
	            {
	            	yoneticiModel musteri=new yoneticiModel(rs.getString("kullanici_adi"),rs.getString("sifre"));
	...
	public boolean control(yoneticiModel yonetici) {
		...
		try {
			...
			String sorgu="SELECT kullanici_adi,sifre FROM yonetici";
 		   	Class.forName("com.mysql.jdbc.Driver");
 		    con=DriverManager.getConnection("jdbc:mysql://localhost:3306/eticaret","root","");
            ps=con.prepareStatement(sorgu);		            
            ResultSet rs=ps.executeQuery();
		...
		public void Add(yoneticiModel yonetici){
		 	   ...
		 	   try {
		 		   String sorgu="INSERT INTO yonetici(kullanici_adi,sifre) VALUES(?,?)";
		 		   Class.forName("com.mysql.jdbc.Driver");
		 		    con=DriverManager.getConnection("jdbc:mysql://localhost:3306/eticaret","root","");
		            ps=con.prepareStatement(sorgu);
			...
			public void Remove(yoneticiModel yonetici){
				...
				try {
					String sorgu="DELETE FROM yonetici WHERE kullanici_adi=?";
					Class.forName("com.mysql.jdbc.Driver");
					con=DriverManager.getConnection("jdbc:mysql://localhost:3306/eticaret","root","");
					ps=con.prepareStatement(sorgu);	    
		            ps.setString(1,yonetici.getKullaniciAdi());
	...
	public void Update(yoneticiModel yonetici){
		...
		try {
			String sorgu="UPDATE yonetici SET sifre=? WHERE kullanici_adi=?";
			Class.forName("com.mysql.jdbc.Driver");
			con=DriverManager.getConnection("jdbc:mysql://localhost:3306/eticaret","root","");
			ps=con.prepareStatement(sorgu);	    
